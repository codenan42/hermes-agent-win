...
async def vision_analyze_tool(
    ...
    try:
        ...
        image_url_expanded = os.path.expanduser(image_url)
        local_path = Path(image_url_expanded)
        if local_path.is_file():
            # Block access to sensitive paths
            if _is_path_denied(image_url_expanded):
                return json.dumps({
                    "success": False,
                    "error": f"Access denied: '{image_url}' is a protected system/credential file.",
                    "analysis": f"Access denied: '{image_url}' is a protected system/credential file."
                })
