...
def _apply_delete(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    # Use the unified delete_file method which includes path security guards
    if not file_ops.delete_file(op.file_path):
        return False, f"Failed to delete {op.file_path} (access denied or command failed)"
...
def _apply_move(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    # Use the unified move_file method which includes path security guards
    if not file_ops.move_file(op.file_path, op.new_path):
        return False, f"Failed to move {op.file_path} to {op.new_path} (access denied or command failed)"
