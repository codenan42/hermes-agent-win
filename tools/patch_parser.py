...
def _apply_delete(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    # Import security check here to avoid circular dependencies
    from tools.file_operations import _is_path_denied

    # Block deletions of sensitive paths
...
def _apply_move(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    # Block moves of sensitive paths (both source and destination)
    if _is_path_denied(op.file_path):
        return False, f"Move denied: '{op.file_path}' is a protected system/credential file."
    if _is_path_denied(op.new_path):
        return False, f"Move denied: '{op.new_path}' is a protected system/credential file."
