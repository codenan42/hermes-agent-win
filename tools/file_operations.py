    ...
    try:
        ...
        for prefix in PATH_DENIED_PREFIXES:
            if resolved.startswith(prefix):
                return True
        # Block the directory itself even without the trailing slash
        for prefix in PATH_DENIED_PREFIXES:
            if resolved == prefix.rstrip(os.sep):
                return True
    except Exception:
        # If path resolution fails, deny for safety if it looks suspicious
        if "~" in path or "/etc/" in path or ".ssh" in path:
            return True
    return False
