...
def _is_path_denied(path: str) -> bool:
    ...
    # Robust directory matching: if resolved is a directory, append os.sep
    # so it matches the PREFIX entries correctly.
    check_path = resolved
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block access to sensitive paths
        if _is_path_denied(path):
            return SearchResult(error=f"Access denied: '{path}' is a protected system/credential directory/file.")
