...
def _is_path_denied(path: str) -> bool:
    ...
    for prefix in PATH_DENIED_PREFIXES:
        if resolved.startswith(prefix):
            return True
    # Also check if the path itself is a directory in the denied prefixes
    # (e.g., if prefix is '/home/user/.ssh/', we also want to deny '/home/user/.ssh')
    resolved_with_sep = resolved if resolved.endswith(os.sep) else resolved + os.sep
    for prefix in PATH_DENIED_PREFIXES:
        if resolved_with_sep == prefix:
            return True
    return False
