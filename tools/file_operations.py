...
def _is_path_denied(path: str) -> bool:
    """Return True if path is on the restricted list (read/write denied)."""
    resolved = os.path.realpath(os.path.expanduser(path))
    if resolved in WRITE_DENIED_PATHS:
        return True
    for prefix in WRITE_DENIED_PREFIXES:
        # Check if it's inside the denied directory (prefix includes trailing slash)
        if resolved.startswith(prefix):
            return True
        # Check if it IS the denied directory itself (remove trailing slash)
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block searches to sensitive paths
        if _is_path_denied(path):
            return SearchResult(
                error=f"Search denied: '{path}' is a protected system/credential directory or file.",
                total_count=0
            )
