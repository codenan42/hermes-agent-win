...
# ---------------------------------------------------------------------------
# Path denial system — blocks access to sensitive system/credential files
# ---------------------------------------------------------------------------
...
def _is_path_denied(path: str) -> bool:
    """Return True if path is on the universal deny list."""
    resolved = os.path.realpath(os.path.expanduser(path))
    if resolved in PATH_DENIED_PATHS:
        return True
    # If it's a directory, add a trailing separator for prefix matching
    resolved_with_sep = resolved if resolved.endswith(os.sep) else resolved + os.sep
    for prefix in PATH_DENIED_PREFIXES:
        if resolved.startswith(prefix) or resolved_with_sep == prefix:
            return True
    return False
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block searches of sensitive paths
        if _is_path_denied(path):
            return SearchResult(
                error=f"Access denied: '{path}' is a protected system/credential file.",
                total_count=0
            )
