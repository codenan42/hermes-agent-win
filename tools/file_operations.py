...
def _is_path_denied(path: str) -> bool:
    """Return True if path is on the access deny list (read/write/search/patch)."""
    resolved = os.path.realpath(os.path.expanduser(path))
    if resolved in WRITE_DENIED_PATHS:
        return True
    for prefix in WRITE_DENIED_PREFIXES:
        # Check if the path matches the directory or anything inside it.
        # prefix already ends with os.sep.
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block access to sensitive paths
        if _is_path_denied(path):
            return SearchResult(error=f"Search denied: '{path}' is a protected system/credential path.", total_count=0)
