...
def _is_path_denied(path: str) -> bool:
    ...
    try:
        ...
        for prefix in PATH_DENIED_PREFIXES:
            # Check for exact prefix match (without trailing slash) or startswith
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block access to sensitive paths
        if _is_path_denied(path):
            return SearchResult(error=f"Access denied: '{path}' is a protected directory/file.", total_count=0)
