...
def _is_path_denied(path: Optional[str]) -> bool:
    """Return True if path is on the deny list (fails closed for None)."""
    if path is None:
        return True
    resolved = os.path.realpath(os.path.expanduser(path))
    if resolved in WRITE_DENIED_PATHS:
        return True
    # Ensure directory matches itself (e.g. ~/.ssh matches ~/.ssh/)
    resolved_dir = resolved if resolved.endswith(os.sep) else resolved + os.sep
    for prefix in WRITE_DENIED_PREFIXES:
        if resolved.startswith(prefix) or resolved_dir == prefix:
            return True
    return False
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block searches from sensitive paths
        if _is_path_denied(path):
