...
# ---------------------------------------------------------------------------
# Path-based access control — blocks operations on sensitive system/credential files
# ---------------------------------------------------------------------------
...
def _is_path_denied(path: str) -> bool:
    ...
    resolved = os.path.realpath(os.path.expanduser(path)).rstrip(os.sep)
    if resolved in PATH_DENIED_PATHS:
        return True
    # For directory prefixes, we want to match both the directory itself
    # and any path inside it.
    for prefix in PATH_DENIED_PREFIXES:
        if (resolved + os.sep).startswith(prefix):
            return True
    return False
...
class FileOperations(ABC):
    ...
    @abstractmethod
    def move_file(self, source: str, destination: str) -> WriteResult:
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block searches in sensitive paths
        if _is_path_denied(path):
    ...
    def delete_file(self, path: str) -> WriteResult:
        """Delete a file using shell rm command."""
        path = self._expand_path(path)
        if _is_path_denied(path):
            return WriteResult(error=f"Delete denied: '{path}' is a protected system/credential path.")

        result = self._exec(f"rm -f {self._escape_shell_arg(path)}")
        if result.exit_code != 0:
            return WriteResult(error=f"Failed to delete file: {result.stdout}")

        return WriteResult(bytes_written=0)  # Indicate success

    def move_file(self, source: str, destination: str) -> WriteResult:
        """Move or rename a file using shell mv command."""
        source = self._expand_path(source)
        destination = self._expand_path(destination)

        if _is_path_denied(source):
            return WriteResult(error=f"Move denied: source '{source}' is a protected system/credential path.")
        if _is_path_denied(destination):
            return WriteResult(error=f"Move denied: destination '{destination}' is a protected system/credential path.")

        result = self._exec(f"mv {self._escape_shell_arg(source)} {self._escape_shell_arg(destination)}")
        if result.exit_code != 0:
            return WriteResult(error=f"Failed to move file: {result.stdout}")
