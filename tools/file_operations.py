...
def _is_path_denied(path: str) -> bool:
    """Return True if path is on the path-based deny list."""
    resolved = os.path.realpath(os.path.expanduser(path))
    if resolved in WRITE_DENIED_PATHS:
        return True
    for prefix in WRITE_DENIED_PREFIXES:
        # Check if the resolved path exactly matches the prefix (blocking the
        # directory itself) or starts with the prefix + separator (blocking
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        if _is_path_denied(path):
            return SearchResult(error=f"Search denied: '{path}' is a protected system/credential directory.")
