...
def _is_path_denied(path: str) -> bool:
    ...
    try:
        ...
        for prefix in PATH_DENIED_PREFIXES:
            # Check for exact directory match OR subpath
            if resolved == prefix.rstrip(os.sep) or resolved.startswith(prefix):
                return True
        return False
    except Exception:
        # If path resolution fails, deny access to be safe
        return True
...
class ShellFileOperations(FileOperations):
    ...
    def _search_files(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        for line in result.stdout.strip().split('\n'):
            ...
            # Filter protected files from results
