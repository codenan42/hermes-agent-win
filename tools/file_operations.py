...
# ---------------------------------------------------------------------------
# Path access control — blocks read/write/search to sensitive system/credential files
# ---------------------------------------------------------------------------
...
def _is_path_denied(path: str) -> bool:
    ...
    try:
        ...
        # Fallback for paths that can't be resolved or other errors
        return True
...
class ShellFileOperations(FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # Block searching in sensitive paths
        if _is_path_denied(path):
            return SearchResult(
                error=f"Search denied: '{path}' is a protected directory/file.",
                total_count=0
            )
