...
class TestLoadSkillData:
    def test_reads_frontmatter_description(self, tmp_path):
        ...
        assert data["frontmatter"].get("name") == "test-skill"
        assert data["description"] == "A useful test skill"
    ...
    def test_nonexistent_file_returns_defaults(self, tmp_path):
        data = _load_skill_data(tmp_path / "missing.md")
        assert data["is_compatible"] is True
    ...
    def test_returns_frontmatter_with_prerequisites(self, tmp_path, monkeypatch):
        ...
        data = _load_skill_data(skill_file)
        assert data["frontmatter"]["prerequisites"]["env_vars"] == ["NONEXISTENT_KEY_ABC"]
