...
# =========================================================================
# _load_skill_data — consolidated skill file reading
# =========================================================================
...
class TestLoadSkillData:
    def test_reads_frontmatter_description(self, tmp_path):
        ...
        is_compat, frontmatter, conditions, desc = _load_skill_data(skill_file)
    ...
    def test_long_description_truncated(self, tmp_path):
        ...
        _, _, _, desc = _load_skill_data(skill_file)
        assert len(desc) <= 60
        assert desc.endswith("...")

    def test_nonexistent_file_returns_defaults(self, tmp_path):
        is_compat, frontmatter, conditions, desc = _load_skill_data(tmp_path / "missing.md")
        assert is_compat is True
        assert frontmatter == {}
        assert conditions == {}
        assert desc == ""
    ...
    def test_returns_conditions(self, tmp_path):
        skill_file = tmp_path / "SKILL.md"
        skill_file.write_text(
            "---\nname: gated\ndescription: Gated skill\n"
            "metadata:\n  hermes:\n    requires_tools: [terminal]\n---\n"
        )
        _, _, conditions, _ = _load_skill_data(skill_file)
