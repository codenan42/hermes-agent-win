...
class TestWriteDenyPrefixes:
    ...
    def test_directory_prefix_itself_blocked(self):
        # The directory itself should be blocked, not just files inside
        path = os.path.join(str(Path.home()), ".ssh")
        assert _is_path_denied(path) is True

    def test_etc_sudoers_d_itself_blocked(self):
        assert _is_path_denied("/etc/sudoers.d") is True
