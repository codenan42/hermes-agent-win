...
class TestPathDenyDirectory:
    def test_ssh_directory_denied(self):
        path = os.path.join(str(Path.home()), ".ssh")
        assert _is_path_denied(path) is True

    def test_ssh_directory_with_slash_denied(self):
        path = os.path.join(str(Path.home()), ".ssh") + os.sep
        assert _is_path_denied(path) is True

class TestReadSearchDeny:
    def test_read_file_denied(self, tmp_path):
        ...
        result = ops.read_file(sensitive_path)
        assert "Read denied" in result.error
        assert result.content == ""

    def test_search_denied(self, tmp_path):
        from tools.file_operations import ShellFileOperations
        from unittest.mock import MagicMock
        env = MagicMock()
        ops = ShellFileOperations(env)

        sensitive_path = os.path.join(str(Path.home()), ".ssh")
        result = ops.search("pattern", path=sensitive_path)
