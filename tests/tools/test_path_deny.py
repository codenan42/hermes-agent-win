...
class TestShellFileOperationsPathDeny:
    ...
    def test_read_file_denied(self, mock_env):
        ops = ShellFileOperations(mock_env)
        path = os.path.join(str(Path.home()), ".ssh", "id_rsa")
        result = ops.read_file(path)
        assert "Access denied" in result.error

    def test_write_file_denied(self, mock_env):
        ops = ShellFileOperations(mock_env)
        path = os.path.join(str(Path.home()), ".ssh", "id_rsa")
        result = ops.write_file(path, "content")
        assert "Access denied" in result.error

    def test_patch_replace_denied(self, mock_env):
        ops = ShellFileOperations(mock_env)
        path = os.path.join(str(Path.home()), ".ssh", "id_rsa")
        result = ops.patch_replace(path, "old", "new")
        assert "Access denied" in result.error

    def test_search_denied_path(self, mock_env):
        ops = ShellFileOperations(mock_env)
        path = os.path.join(str(Path.home()), ".ssh")
        result = ops.search("pattern", path=path)
        assert "Access denied" in result.error
