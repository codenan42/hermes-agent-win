...
class TestIsPathDenied:
    ...
    def test_none_path_denied(self):
        assert _is_path_denied(None) is True

    def test_directory_match_without_slash(self):
        # ~/.ssh/ is a prefix, so ~/.ssh should also be denied
...
class TestShellFileOpsPathDenied:
    ...
    def test_search_denied_path(self, file_ops):
        result = file_ops.search("pattern", path="~/.ssh/id_rsa")
        assert result.error is not None
