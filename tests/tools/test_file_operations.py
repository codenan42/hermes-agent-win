...
class TestShellFileOpsPathDenied:
    def test_read_file_denied_path(self, file_ops):
        ...
        assert result.error is not None
        assert "read denied" in result.error.lower()

    def test_write_file_denied_path(self, file_ops):
        result = file_ops.write_file("~/.ssh/authorized_keys", "evil key")
        assert result.error is not None
        assert "write denied" in result.error.lower()
    ...
    def test_search_denied_path(self, file_ops, mock_env):
        # We need to mock _exec's path existence check to return success
        # and mock 'command -v' to find 'rg' so it doesn't fail on missing binary.
        # Also mock 'echo $HOME' so path expansion matches the local machine's home
        # (which is what _is_path_denied uses to build its prefix list).
        from tools.file_operations import _HOME
        def side_effect(command, **kwargs):
            if "test -e" in command:
                return {"output": "exists", "returncode": 0}
            if "command -v" in command:
                return {"output": "/usr/bin/rg", "returncode": 0}
            if "echo $HOME" in command:
                return {"output": _HOME, "returncode": 0}
            return {"output": "", "returncode": 0}
        mock_env.execute.side_effect = side_effect

        result = file_ops.search("pattern", path="~/.ssh")
        assert result.error is not None
        assert "search denied" in result.error.lower()
