...
class TestShellFileOpsPathDenied:
    ...
    def test_delete_denied_path(self, file_ops):
        result = file_ops.delete_file("~/.ssh/id_rsa")
        assert result.exit_code != 0
        assert "denied" in result.stdout.lower()

    def test_move_denied_src(self, file_ops):
        result = file_ops.move_file("~/.ssh/id_rsa", "/tmp/stolen_key")
        assert result.exit_code != 0
        assert "denied" in result.stdout.lower()

    def test_move_denied_dst(self, file_ops):
        result = file_ops.move_file("/tmp/evil_config", "/etc/shadow")
        assert result.exit_code != 0
        assert "denied" in result.stdout.lower()
