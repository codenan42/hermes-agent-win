...
def _load_skill_data(skill_file: Path) -> tuple[bool, str, dict]:
    """Read a SKILL.md once and return (is_compatible, description, conditions).

    Consolidates I/O and parsing for both platform checks, descriptions,
    and conditional activation rules.
    """
    try:
        from tools.skills_tool import _parse_frontmatter, skill_matches_platform

        # Consolidate: Read and parse once
        ...
        return True, desc, conditions
    except Exception as e:
        logger.debug("Failed to load skill data for %s: %s", skill_file, e)
        # On error, default to showing the skill with no special conditions
        return True, "", {}
...
def build_skills_system_prompt(
    ...
    for skill_file in skills_dir.rglob("SKILL.md"):
        # Consolidate: Single call to load metadata and check compatibility
        is_compatible, desc, conditions = _load_skill_data(skill_file)
