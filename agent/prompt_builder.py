...
def _load_skill_data(skill_file: Path) -> dict:
    ...
    On error, returns defaults that err on the side of showing the skill.
    """
    try:
        ...
        # 2. Description (compact for index)
        desc = ""
...
def _parse_skill_file(skill_file: Path) -> tuple[bool, dict, str]:
    """Deprecated: use _load_skill_data. Kept for test compatibility."""
...
def build_skills_system_prompt(
    ...
    for skill_file in skills_dir.rglob("SKILL.md"):
        # Load all metadata in one pass (single file read and parse)
        data = _load_skill_data(skill_file)
